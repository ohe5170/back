            ...
            passInput.addEventListener("keyup", (e) => {
                ...
                if (!regCheck) {
                    ...
                    inputWorngs[i].innerHTML =
                        "대문자, 소문자, 숫자, 특수문자를 최소 1개씩 포함하여, 8~15자리를 입력해주세요.";
...
userPasswordSubmitBtn.addEventListener("click", async (e) => {
    const prevPw = prevPasswordInput.value;
    const newPw = passwordInput.value;
    const newPwCheck = passwordCheckInput.value;

    if (!prevPw || !newPw || !newPwCheck) {
        alert("모든 항목을 입력해주세요.");
        return;
    }
    if (newPw !== newPwCheck) {
    ...
    // 현재 비밀번호 확인
    const checkResponse = await profileService.checkPassword(prevPw);
    const isCorrect = await checkResponse.json();
    if (!isCorrect) {
        alert("현재 비밀번호가 일치하지 않습니다.");
        return;
    }
    // 비밀번호 변경
    const response = await profileService.updatePassword(newPw);
