...
document.addEventListener("DOMContentLoaded", (e) => {
    ...
    // --------------------------------------------------------------------------
    // 상단 클릭 부분 누르면 해당 탭 효과
    upperClickContents.forEach((content, i) => {
        content.addEventListener("click", (e) => {
            console.log(content);
            switch (i) {
                case 0:
                    document
                        .querySelector('.Navigation-Span[name="review"]')
                        ?.click();
                    break;
                case 1:
                    document
                        .querySelector('.Navigation-Span[name="likeItem"]')
                        ?.click();
                    break;
                case 2:
                    document
                        .querySelector('.Navigation-Span[name="pending"]')
                        ?.click();
                    break;
                case 3:
                    document
                        .querySelector('.Navigation-Span[name="complete"]')
                        ?.click();
                    break;
            }
        });
